...
public class MySwerve extends CustomDrivetrain {
     ...
     */
    public MySwerve(HardwareMap hardwareMap, SwerveConstants constants, SwervePod... pods) {
    ...
    public void arcadeDrive(double forward, double strafe, double rotation) {
        ...
        for (int podNum = 0; podNum < pods.size(); podNum++) {
            ...
            double err= normalize(finalVector.getTheta())- normalize(pods.get(podNum).getAngle());
            double alignmentFactor= Math.abs(err)< Math.PI/4 ? 1-err/Math.PI : 1;

            pods.get(podNum).move(finalVector.getTheta(), finalVector.getMagnitude() * avgScaling * alignmentFactor,
                    zeroTrans && zeroRotation && constants.getZeroPowerBehavior() == SwerveConstants.ZeroPowerBehavior.IGNORE_ANGLE_CHANGES);
        }
    }

    private double normalize(double h){
        while (h< Math.PI/2) h+= Math.PI;
        while (h>= Math.PI/2) h-=Math.PI;
        return h;
    }
    /**
